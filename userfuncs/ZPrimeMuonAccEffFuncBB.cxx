/***************************************************************************** 
 * Project: RooFit                                                           * 
 *                                                                           * 
 * This code was autogenerated by RooClassFactory                            * 
 *****************************************************************************/ 

// Your description goes here... 

#include "Riostream.h" 

// class declaration include file below retrieved from workspace code storage
#include "ZPrimeMuonAccEffFuncBB.h"
#include "RooAbsReal.h" 
#include "RooAbsCategory.h" 
#include <math.h> 
#include "TMath.h" 

ClassImp(ZPrimeMuonAccEffFuncBB) 

ZPrimeMuonAccEffFuncBB::ZPrimeMuonAccEffFuncBB(const char *name, const char *title, 
					   RooAbsReal& zPrimeMass,
					   RooAbsReal& effScale,
					   RooAbsReal& effA,
					   RooAbsReal& effB,
					   RooAbsReal& effC,
					   RooAbsReal& trigA,
                                           RooAbsReal& trigB,
                                           RooAbsReal& trigC):
  RooAbsReal(name,title), 
  zPrimeMass_("zPrimeMass","zPrimeMass",this,zPrimeMass),
  effScale_("eff_scale","eff_scale",this,effScale),
  effA_("eff_a","eff_a",this,effA),
  effB_("eff_b","eff_b",this,effB),
  effC_("eff_c","eff_c",this,effC),
  trigA_("trig_a","trig_a",this,trigA),
  trigB_("trig_b","trig_b",this,trigB),
  trigC_("trig_c","trig_c",this,trigC)
{ 
} 


ZPrimeMuonAccEffFuncBB::ZPrimeMuonAccEffFuncBB(const ZPrimeMuonAccEffFuncBB& other, const char* name) :  
  RooAbsReal(other,name), 
  zPrimeMass_("zPrimeMass",this,other.zPrimeMass_),
  effScale_("eff_scale",this,other.effScale_),
  effA_("eff_a",this,other.effA_),
  effB_("eff_b",this,other.effB_),
  effC_("eff_c",this,other.effC_),
  trigA_("trig_c",this,other.trigA_),
  trigB_("trig_c",this,other.trigB_),
  trigC_("trig_c",this,other.trigC_)
{ 
} 



Double_t ZPrimeMuonAccEffFuncBB::evaluate() const 
{ 
  // ENTER EXPRESSION IN TERMS OF VARIABLE ARGUMENTS HERE 
  return effScale_*(effA_+effB_/zPrimeMass_ + effC_*zPrimeMass_)*(trigA_ + trigB_*zPrimeMass_ + trigC_*zPrimeMass_*zPrimeMass_); 
} 



